...
import { Card, Statistic, Tooltip, Space, Tag } from 'antd';
import {
  FileOutlined,
  BranchesOutlined,
  ClockCircleOutlined,
...
export interface ProcessKPIs {
  ...
  avgThroughputTime?: number;  // in seconds
  medianThroughputTime?: number;
  ...
  return (
    ...
    >
      ...
      {conformanceScore !== undefined && (
        <KPICard
          title="Conformance"
          value={conformanceScore.toFixed(0)}
          suffix="%"
          icon={<CheckCircleOutlined />}
          tooltip="How well the process follows the expected model"
          trend={trends?.conformanceScore}
          color={conformanceScore >= 90 ? COLORS.performance.good : conformanceScore >= 70 ? COLORS.performance.moderate : COLORS.performance.poor}
          compact={compact}
        />
      )}
    </div>
