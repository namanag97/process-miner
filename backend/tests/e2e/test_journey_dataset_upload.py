...
class TestDatasetUploadJourney:
    ...
    async def test_03_detect_columns(
        ...
        assert len(columns) >= 3, (
            f"Expected at least 3 columns (case_id, activity, timestamp), "
            f"but got {len(columns)}"
        )
        ...
        for col in columns:
            ...
            role_info = f" (suggested: {suggested_role}, confidence: {confidence:.2f})" if suggested_role else ""
    ...
    async def test_04_submit_column_mapping(
        ...
        print(f"✓ Column mapping submitted successfully")
    ...
    async def test_06_trigger_ingestion(
        ...
        if "job_id" in ingest_data:
            ...
            print(f"✓ Ingestion triggered (async mode)")
            print(f"  Job ID: {job_id}")
        else:
            print(f"✓ Ingestion triggered (sync mode)")
    ...
    async def test_07_wait_for_ingestion_complete(
        ...
        assert final_state.lower() == "ready", (
            f"Expected final state READY, got: {final_state}"
        )
    ...
    async def test_08_verify_dataset_statistics(
        ...
        print(f"✓ Dataset statistics verified")
        ...
        assert events_per_case > 1, (
            f"Each case should have multiple events. "
            f"Got {events_per_case:.2f} events per case"
        )
