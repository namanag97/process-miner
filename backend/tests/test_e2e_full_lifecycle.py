...
"""
    ...
    async def test_full_user_lifecycle(
        ...
        proj_resp = await client.post(
            f"/api/v1/projects?workspace_id={workspace_id}", json=project_payload
        )
        ...
        print(
            f"   📊 Stats: {dataset.get('total_cases')} cases, {dataset.get('total_events')} events"
        )
        ...
        for edge in edges:
            ...
            assert "frequency" in edge, (
                "CONTRACT VIOLATION: Edge missing 'frequency' (used for weight)"
            )
            assert isinstance(edge["source"], str), (
                "CONTRACT VIOLATION: Edge 'source' must be string"
            )
            assert isinstance(edge["target"], str), (
                "CONTRACT VIOLATION: Edge 'target' must be string"
            )
        ...
        conf_resp = await client.post(
            ...
            json={"dataset_id": dataset_id, "model_id": model_id, "method": "token_replay"},
        )
