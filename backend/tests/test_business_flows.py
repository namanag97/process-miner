...
class TestProcessDiscovery:
    ...
    @pytest.mark.asyncio
    async def test_dfg_edge_probabilities_valid(
...
class TestConformanceChecking:
    ...
    async def test_comprehensive_quality_has_all_dimensions(
        ...
        check_resp = await client.post(
            "/api/v1/conformance/check",
            json={"log_id": uploaded_log_id, "model_id": discovered_model_id, "method": "token_replay"}
        )
...
class TestPerformanceAnalysis:
    ...
    @pytest.mark.asyncio
    async def test_activity_durations_non_negative(
        self, client: AsyncClient, uploaded_log_id: str
    ):
...
class TestOrganizationalMining:
    ...
    @pytest.mark.asyncio
    async def test_handover_network_structure(
        self, client: AsyncClient, uploaded_log_id: str
    ):
...
class TestPM4PyAnalytics:
    ...
    async def test_case_duration_statistics(self, client: AsyncClient, uploaded_log_id: str):
        ...
        response = await client.get(
            f"/api/v1/process-mining/duration-stats/{uploaded_log_id}"
        )
        assert response.status_code == 200
        stats = response.json()
    ...
    async def test_start_and_end_activities(self, client: AsyncClient, uploaded_log_id: str):
        ...
        start_resp = await client.get(
            f"/api/v1/process-mining/start-activities/{uploaded_log_id}"
        )
        assert start_resp.status_code == 200
        
        end_resp = await client.get(
            f"/api/v1/process-mining/end-activities/{uploaded_log_id}"
        )
...
class TestEndToEndIntegration:
    ...
    @pytest.mark.asyncio
    async def test_model_crud_operations(
        self, client: AsyncClient, uploaded_log_id: str
    ):
