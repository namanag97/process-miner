...
.. deprecated::
    This router is deprecated. Use `/processes` endpoints instead.
    The `/processes` router provides unified access to both traditional
    event logs and object-centric (OCEL 2.0) data.
"""
...
from src.presentation.api.schemas import (
    ...
    StatisticsResponse,
    UpdateRequest,
    VariantResponse as SharedVariantResponse,
)
...
router = APIRouter(prefix="/logs", tags=["Event Logs (Deprecated)"])
...
# Use shared VariantResponse - local alias maintains backward compatibility
class VariantResponse(BaseModel):
    """Simplified variant response for logs router."""
    key: str
    activities: List[str]
    case_count: int
    length: int
...
# Use shared LogStatisticsResponse (alias for StatisticsResponse)
LogStatisticsResponse = StatisticsResponse


# Reuse shared QualityIssueResponse and QualityReportResponse are already imported
# Define local alias for backward compatibility
LogQualityReportResponse = QualityReportResponse

# Alias for LogUpdateRequest
LogUpdateRequest = UpdateRequest
...
async def preview_file(
    ...
    try:
        ...
        if file_format == "csv":
            ...
            # Estimate counts
            len(sample_rows) if len(sample_rows) < 10 else "10+ (preview)"
        ...
        raise HTTPException(status_code=HttpStatus.BAD_REQUEST, detail=f"Preview failed: {str(e)}")
...
async def get_log_statistics(
    ...
    try:
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Statistics calculation failed: {str(e)}")
...
async def get_log_quality(
    ...
    try:
        ...
        validity_score = (
            1.0
            if duplicate_count == 0
            else max(0.5, 1.0 - (duplicate_count / max(log.total_events, 1)))
        )
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Quality check failed: {str(e)}")
...
async def get_enhanced_variants(
    ...
    try:
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Variant analysis failed: {str(e)}")
