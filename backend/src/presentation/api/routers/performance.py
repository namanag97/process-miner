    ...
    if avg_durations:
        ...
        for metric in activity_metrics:
            if (
                ...
                and metric["avg_duration_seconds"] > duration_threshold
            ):
                severity = min(
                    1.0, (metric["avg_duration_seconds"] - duration_mean) / duration_mean
                )
                bottlenecks.append(
                    {
                        "id": generate_uuid(),
    ...
    if transition_times:
        ...
        for metric in transition_metrics:
            if metric["avg_time_seconds"] and metric["avg_time_seconds"] > transition_threshold:
                severity = min(
                    1.0, (metric["avg_time_seconds"] - transition_mean) / transition_mean
                )
                bottlenecks.append(
                    {
                        "id": generate_uuid(),
...
async def get_performance_summary(
    ...
    try:
        ...
        stmt = (
            select(PerformanceAnalysisRunModel)
            .where(PerformanceAnalysisRunModel.log_id == str(log_id))
            .order_by(PerformanceAnalysisRunModel.created_at.desc())
            .limit(1)
        )
        ...
        if latest_run:
            stmt = (
                select(BottleneckFindingModel)
                .where(BottleneckFindingModel.analysis_run_id == latest_run.id)
                .order_by(BottleneckFindingModel.severity_score.desc())
                .limit(5)
            )
            result = await session.execute(stmt)
        ...
        return PerformanceSummaryResponse(
            ...
            analyzed_at=latest_run.completed_at.isoformat()
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Performance summary failed: {str(e)}")
...
async def get_duration_histogram(
    ...
    **Parameters:**
    ...
    if not durations:
        raise HTTPException(status_code=HttpStatus.BAD_REQUEST, detail="No case durations available")
...
async def get_transition_performance(
    ...
    if latest_run:
        # Return from stored metrics
        stmt = (
            select(TransitionPerformanceMetricModel)
            .where(TransitionPerformanceMetricModel.analysis_run_id == latest_run.id)
            .order_by(TransitionPerformanceMetricModel.transition_count.desc())
            .limit(limit)
        )
