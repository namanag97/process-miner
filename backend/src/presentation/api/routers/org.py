...
async def calculate_handovers(log) -> List[Dict[str, Any]]:
    ...
    for case in log.cases:
        ...
        for i in range(len(sorted_events) - 1):
            ...
            to_resource = (
                str(sorted_events[i + 1].resource) if sorted_events[i + 1].resource else None
            )

            if (
                from_resource
                and to_resource
                and from_resource != to_resource
                and from_resource != "None"
                and to_resource != "None"
            ):
                ...
                try:
                    time_diff = (
                        sorted_events[i + 1].timestamp.value - sorted_events[i].timestamp.value
                    ).total_seconds()
                    if time_diff >= 0:
            ...
            roles.append(
                {
                    ...
                    "role_name": f"Role {idx + 1}"
                    if len(signature) == 0
                    else f"Role: {', '.join(list(signature)[:3])}...",
                    "resources": members,
...
async def get_resources(
    ...
    sort_by: str = Query(
        default="total_activities", pattern="^(total_activities|total_cases|identifier)$"
    ),
    ...
    **Sort Options:**
    ...
    try:
        ...
        stmt = select(ResourceProfileModel).where(ResourceProfileModel.log_id == str(log_id))
        ...
        if existing:
            ...
            for r in existing:
                frequent = (
                    json.loads(r.frequent_activities_json) if r.frequent_activities_json else []
                )
                resources.append(
                    ResourceResponse(
                        id=r.id,
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Resource extraction failed: {str(e)}")
...
async def get_handover_network(
    ...
    **Filters:**
    ...
    try:
        ...
        stmt = (
            select(HandoffModel)
            .where(HandoffModel.log_id == str(log_id), HandoffModel.frequency >= min_frequency)
            .order_by(HandoffModel.frequency.desc())
            .limit(limit)
        )
        result = await session.execute(stmt)
        existing = list(result.scalars())
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Handover network discovery failed: {str(e)}")
...
async def get_working_together_network(
    ...
    **Filters:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=HttpStatus.INTERNAL_ERROR, detail=f"Working together network discovery failed: {str(e)}"
        )
...
async def discover_organizational_roles(
    ...
    try:
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Role mining failed: {str(e)}")
...
async def get_resource_workload(
    ...
    try:
        ...
        raise HTTPException(status_code=HttpStatus.INTERNAL_ERROR, detail=f"Workload analysis failed: {str(e)}")
