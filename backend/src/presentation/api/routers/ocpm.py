...
async def list_ocel_logs(
    ...
    for log in logs:
        ...
        response_logs.append(OCELLogResponse(
            id=log.id,
            name=log.name,
...
async def get_object_types(
    ...
    return [
        ObjectTypeResponse(
            ...
            attributes=json.loads(ot.attributes_schema_json).keys() if ot.attributes_schema_json else [],
        )
...
async def discover_oc_petri_net(
    ...
    """
    result = await session.execute(
        select(OCELLogModel).where(OCELLogModel.id == request.log_id)
    )
    log = result.scalar_one_or_none()
