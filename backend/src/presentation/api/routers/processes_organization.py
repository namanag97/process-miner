...
async def get_resources(
    ...
    for r_id, data in list(resource_data.items())[:limit]:
        ...
        resources.append(ResourceResponse(
            id=r_id, identifier=r_id, name=None, role=None,
...
async def get_handover_network(
    ...
    edges = [
        ...
        for (r1, r2, act), freq in handovers.items() if freq >= min_frequency
    ]
    
    return HandoverNetworkResponse(
        process_id=str(process_id), total_resources=len(resources),
        total_handovers=len(edges),
        nodes=[{"id": r, "label": r} for r in resources], edges=edges
    )
...
async def get_working_together(
    ...
    edges = [
        ...
        for p, c in together.items() if c >= min_shared_cases
    ]
...
async def discover_roles(
    ...
    for i, (acts, members) in enumerate(role_clusters.items()):
        roles.append(RoleCluster(
            role_id=f"role_{i+1}",
            role_name=f"Role {i+1}" if len(acts) > 3 else "-".join(acts[:3]),
            resources=members, common_activities=list(acts)
        ))
    
    return RoleMiningResponse(
        process_id=str(process_id), total_resources=len(resource_acts),
        discovered_roles=len(roles), roles=roles
    )
...
async def get_workload(
    ...
    if not workload:
        return WorkloadResponse(
            process_id=str(process_id), resources=[],
            avg_activities_per_resource=0, max_workload_resource=None
        )
    ...
    return WorkloadResponse(
        process_id=str(process_id), resources=resources,
        avg_activities_per_resource=avg, max_workload_resource=max_r
    )
