...
class BusinessUseCases:
    ...
    def _generate_p2p_recommendations(
        ...
        if not recommendations:
            recommendations.append(
                "Process is compliant. Continue monitoring for process drift."
            )
    ...
    def split_log_by_attribute(
        ...
        return {
            ...
            "total_cases": len(pm4py_log),
            "filtered_cases": len(filtered_cases),
            "percentage": round(len(filtered_cases) / len(pm4py_log) * 100, 2)
            if pm4py_log
            else 0,
    ...
    def compare_process_variants(
        ...
        durations1 = [
            ...
            for trace in log1
            if len(trace) > 0
            and "time:timestamp" in trace[0]
            and "time:timestamp" in trace[-1]
        ]
        ...
        return {
            ...
            "differences": {
                ...
                "duration_difference_pct": round(
                    ((avg_duration2 - avg_duration1) / avg_duration1 * 100) if avg_duration1 > 0 else 0,
                    2,
                ),
    ...
    def simulate_process_changes(
        ...
        for trace in pm4py_log:
            if (
                ...
                duration = (trace[-1]["time:timestamp"] - trace[0]["time:timestamp"]).total_seconds() / 3600
        ...
        simulated_avg = sum(simulated_durations) / len(simulated_durations) if simulated_durations else 0
        simulated_p50 = sorted(simulated_durations)[len(simulated_durations) // 2] if simulated_durations else 0
        simulated_p95 = sorted(simulated_durations)[int(len(simulated_durations) * 0.95)] if simulated_durations else 0
        ...
        return {
            ...
            "impact_analysis": {
                ...
                "duration_reduction_pct": round(
                    ((baseline_avg - simulated_avg) / baseline_avg * 100) if baseline_avg > 0 else 0,
                    2,
                ),
