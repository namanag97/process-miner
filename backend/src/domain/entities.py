...
class DatasetAggregate:
    ...
    def __init__(
        ...
        # Caches (lazy-loaded)
        self._pm4py_cache: Optional["PM4PyLog"] = None
        self._variants_cache: Optional[Dict[ActivitySequence, List[ProcessCase]]] = None
        self._statistics_cache: Optional[ProcessStatistics] = None
        self._activities_cache: Optional[set[str]] = None
    ...
    def activities(self) -> set[str]:
        ...
        if self._activities_cache is None:
            self._activities_cache = {
                e.activity for c in self._cases for e in c.events
            }
    ...
    def resources(self) -> set[str]:
        ...
        return {
            e.resource for c in self._cases for e in c.events if e.resource
        }
    ...
    def get_variant_stats(self, top_n: Optional[int] = None) -> List[VariantStats]:
    ...
    def filter_by_activities(
        ...
        def matches(case: ProcessCase) -> bool:
            ...
            if include and not case_activities.issuperset(include):
                return False
            if exclude and case_activities.intersection(exclude):
                return False
            return True
    ...
    def filter_top_variants(self, k: int) -> "DatasetAggregate":
        ...
        return self.filter(
            lambda c: c.events and c.variant in top_variants,
            suffix=f"top{k}_variants"
        )
