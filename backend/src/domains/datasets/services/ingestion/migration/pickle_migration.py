...
from src.features.process_mining.services.serializers import GraphStructureSerializer, PnmlExporter
...
class PickleMigrationService:
    ...
    async def migrate_all_models(
        ...
        try:
            ...
            for model in models:
                migration_result = await self.migrate_single_model(session, model, dry_run=dry_run)
                report.results.append(migration_result)

                if migration_result.success:
                    report.migrated += 1
                elif (
                    migration_result.error and "already migrated" in migration_result.error.lower()
                ):
                    report.skipped += 1
                else:
                    report.failed += 1
                    if migration_result.error:
                        report.errors.append(f"Model {model.id}: {migration_result.error}")
    ...
    def _export_petri_net(self, model_data: Any, model_id: str) -> dict[str, Any] | None:
    ...
    def _serialize_dfg(self, model_data: Any) -> dict[str, Any] | None:
        ...
        try:
            ...
            if isinstance(model_data, dict) and all(
                ...
                return self.graph_serializer.serialize_dfg(model_data, {}, {})
