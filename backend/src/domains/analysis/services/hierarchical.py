...
class HierarchicalMiningService:
    ...
    def apply_activity_mapping(self, log: EventLog, mapping_rules: dict[str, str]) -> EventLog:
    ...
    def discover_hierarchical_models(
        self, log: EventLog, mapping_rules: dict[str, str], miner_type: str = "inductive"
        ...
        if mapping_rules:
            ...
            dfg_mapped, start_mapped, end_mapped, act_count_mapped = (
                self.mining_service.discover_dfg(mapped_log)
            )
            graph_json_mapped = self.serializer.serialize_dfg(
                dfg_mapped, start_mapped, end_mapped, act_count_mapped
            )
    ...
    def suggest_activity_groupings(self, log: EventLog, num_groups: int = 5) -> dict[str, str]:
    ...
    def calculate_drill_down_metadata(
        self, graph_json: dict[str, Any], mapping_rules: dict[str, str]
    ) -> dict[str, Any]:
