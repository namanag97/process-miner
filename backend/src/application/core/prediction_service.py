...
class PredictionService:
    ...
    def predict_remaining_time(
        ...
        for completed_case in event_log.cases:
            if (completed_case.duration and 
                completed_case.case_id != case.case_id and
                self._is_prefix_match(current_activities, completed_case.activities)):
                similar_cases.append(completed_case)
        
        if not similar_cases:
            # No similar cases, use average
            all_durations = [
                c.duration.total_seconds 
    ...
    def get_process_insights(
        ...
        top_variant_coverage = (
            ...
            if event_log.total_cases > 0 else 0
        )
        ...
        return {
            "process_complexity": {
                ...
                "avg_case_length": total_events / event_log.total_cases if event_log.total_cases > 0 else 0,
            },
            "process_standardization": {
                "top_5_variant_coverage": top_variant_coverage,
                "is_highly_standardized": top_variant_coverage > 0.8,
            },
            "activity_distribution": dict(sorted(
                activity_distribution.items(), 
                key=lambda x: -x[1]
            )[:10]),
            "likely_start_activities": dict(sorted(
                start_activities.items(), 
                key=lambda x: -x[1]
            )[:3]),
            "likely_end_activities": dict(sorted(
