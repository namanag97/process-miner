...
from src.domain.value_objects import Gateway, GatewayDirection, GatewayType, Transition
...
class TransitionService:
    ...
    def compute_transitions(
        ...
        for (source, target), frequency in dfg.items():
            probability = (
                frequency / outgoing_totals[source] if outgoing_totals[source] > 0 else 0.0
            )
    ...
    def _compute_duration_statistics(
        ...
        for case in event_log.cases:
            ...
            for i in range(len(events) - 1):
                ...
                duration = (
                    events[i + 1].timestamp.value - events[i].timestamp.value
                ).total_seconds()
