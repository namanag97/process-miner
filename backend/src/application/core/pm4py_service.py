...
from pm4py.algo.evaluation.generalization import algorithm as generalization_evaluator  # noqa: E402
from pm4py.algo.evaluation.simplicity import algorithm as simplicity_evaluator  # noqa: E402
from pm4py.algo.organizational_mining.roles import algorithm as roles_discovery  # noqa: E402
from pm4py.algo.organizational_mining.sna import algorithm as sna  # noqa: E402
from pm4py.objects.petri_net.obj import Marking, PetriNet  # noqa: E402
from pm4py.statistics.traces.generic.log import case_arrival, case_statistics  # noqa: E402

from src.application.core.discovery_service import discovery_service  # noqa: E402
from src.domain.entities import EventLog  # noqa: E402
...
class PM4PyService:
    ...
    def discover_log_skeleton(self, event_log: EventLog) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "always_after": [
                    f"{k[0]} after {k[1]}" for k in list(skeleton.get("always_after", []))[:20]
                ],
                "always_before": [
                    f"{k[0]} before {k[1]}" for k in list(skeleton.get("always_before", []))[:20]
                ],
                "never_together": [
                    f"{k[0]} never with {k[1]}"
                    for k in list(skeleton.get("never_together", []))[:20]
                ],
            }
        except Exception as e:
            return {"error": str(e)}
    ...
    def discover_organizational_roles(self, event_log: EventLog) -> Dict[str, Any]:
        ...
        has_resources = any("org:resource" in event for trace in pm4py_log for event in trace)
            ...
            for role in roles:
                try:
                    if hasattr(role, "resources") and hasattr(role, "activities"):
                        ...
                        roles_data.append(
                            {
                                ...
                                if isinstance(role.resources, (set, list))
                                else [role.resources],
                                "activities": list(role.activities)
                                if isinstance(role.activities, (set, list))
                                else [role.activities],
                            }
                        )
                    else:
                        # Old tuple format
                        roles_data.append(
                            {
                                "resources": list(role[0])
                                if isinstance(role[0], (set, list))
                                else [role[0]],
                                "activities": list(role[1])
                                if isinstance(role[1], (set, list))
                                else [role[1]],
                            }
                        )
                except Exception:
                ...
                for batch in batches[:10]:  # Limit to first 10 batch types
                    try:
                        ...
                        for batch_type, instances in batch_types.items():
                            type_counts[batch_type] = (
                                len(instances) if isinstance(instances, list) else 1
                            )

                        batch_summary.append(
                            {
                                "activity": activity_resource[0]
                                if isinstance(activity_resource, (list, tuple))
                                else str(activity_resource),
                                "resource": activity_resource[1]
                                if isinstance(activity_resource, (list, tuple))
                                and len(activity_resource) > 1
                                else "N/A",
    ...
    def get_comprehensive_analysis(self, event_log: EventLog) -> Dict[str, Any]:
        ...
        # Variants
        results["analyses"]["variants"] = safe_run("variants", lambda: self.get_variants(event_log))

        # DFG
        results["analyses"]["dfg"] = safe_run("dfg", lambda: self.discover_dfg(event_log))
        ...
        # Batches
        results["analyses"]["batches"] = safe_run("batches", lambda: self.detect_batches(event_log))
        ...
        # Summary
        successful = sum(
            1 for a in results["analyses"].values() if isinstance(a, dict) and "error" not in a
        )
        failed = sum(
            1 for a in results["analyses"].values() if isinstance(a, dict) and "error" in a
        )
