...
class WorkflowService:
    ...
    def _register_builtin_pipelines(self):
        ...
        self._pipelines["full_analysis"] = [
            ...
            PipelineStep(
                "discover", "discover_model", {"miner_type": "inductive"}, depends_on=["ingest"]
            ),
            ...
            PipelineStep(
                "report", "generate_report", {}, depends_on=["conformance", "performance"]
            ),
        ]
        ...
        self._pipelines["discovery_only"] = [
            ...
            PipelineStep(
                "inductive", "discover_model", {"miner_type": "inductive"}, depends_on=["ingest"]
            ),
            PipelineStep(
                "heuristics", "discover_model", {"miner_type": "heuristics"}, depends_on=["ingest"]
            ),
        ]
    ...
    def register_step_handler(self, job_type: str, handler: Callable) -> None:
        ...
        self._task_queue.register_handler(job_type, handler)
    ...
    async def _execute_step(
        ...
        # Enqueue job
        job_id = await self._task_queue.enqueue(step.job_type, step_params)

        # Wait for completion (poll)
        while True:
            job = self._task_queue.get_job(job_id)
            if job is None:
