...
async def process_chunk(
    ...
    try:
        ...
        cases_data = result["cases_arrow"].to_pylist() if hasattr(result["cases_arrow"], "to_pylist") else []
        events_data = result["events_arrow"].to_pylist() if hasattr(result["events_arrow"], "to_pylist") else []
            ...
            for event_dict in events_data:
                ...
                if case_ref_id:
                    event = ProcessEvent(
                        ...
                        resource=str(event_dict.get("resource")) if event_dict.get("resource") else None,
                    )
                    db.add(event)
                    events_inserted += 1
