...
async def upload_ocel(db: DBSession, file: UploadFile = File(...), name: str | None = Form(None), async_mode: bool = True):
...
async def list_ocel_logs(db: DBSession):
    """List all OCEL logs."""
...
async def get_ocel_log(dataset_id: str, db: DBSession):
...
async def delete_ocel_log(dataset_id: str, db: DBSession):
    """Delete an OCEL log."""
    result = await db.execute(select(OCELLog).where(OCELLog.id == dataset_id))
    log = result.scalar_one_or_none()
    if not log:
...
async def get_object_types(dataset_id: str, db: DBSession):
...
async def get_ocel_statistics(dataset_id: str, db: DBSession):
...
async def discover_oc_petri_net(request: DiscoverOCPNRequest, db: DBSession, async_mode: bool = True):
...
async def list_oc_petri_nets(db: DBSession):
...
async def get_oc_petri_net(model_id: str, db: DBSession):
...
async def delete_oc_petri_net(model_id: str, db: DBSession):
...
async def get_object_relationships(dataset_id: str, db: DBSession):
...
async def get_oc_dfg(dataset_id: str, db: DBSession):
