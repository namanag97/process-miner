...
class BottleneckAnalyzer:
    ...
    def calculate_waiting_times(self, log: EventLog) -> dict[str, dict[str, Any]]:
    ...
    def identify_bottlenecks(
        self, log: EventLog, threshold_percentile: float = 0.9
    ) -> list[dict[str, Any]]:
    ...
    def calculate_throughput(
        self, log: EventLog, time_window_hours: int = 1
        ...
        logger.info(
            "throughput_calculated", activities=len(throughput), window_hours=time_window_hours
        )

        return {activity: dict(counter) for activity, counter in throughput.items()}

    def calculate_heat_map_data(self, log: EventLog) -> dict[str, float]:
    ...
    def calculate_queue_lengths(self, log: EventLog) -> dict[str, dict[str, Any]]:
