...
class GraphStructureSerializer:
    ...
    def serialize_dfg(
        ...
        result = {
            ...
            "metadata": {"nodeCount": len(nodes), "edgeCount": len(edges), "type": "dfg"},
        }
    ...
    def serialize_petri_net(self, net: "PetriNet", im: "Marking", fm: "Marking") -> dict[str, Any]:
    ...
    def precompute_abstraction_levels(
        ...
        for level in levels:
            ...
            serialized = self.serialize_dfg(
                filtered_dfg, copy.deepcopy(start_activities), copy.deepcopy(end_activities)
            )

            results[level] = serialized

        logger.info("abstraction_levels_computed", levels=levels, original_edges=len(dfg))

        return results

    def add_layout_hints(
        self, graph_json: dict[str, Any], layout_algorithm: str = "dagre"
    ) -> dict[str, Any]:
