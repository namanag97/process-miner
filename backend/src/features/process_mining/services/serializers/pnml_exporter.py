...
class PnmlExporter:
    ...
    def export(self, net: "PetriNet", im: "Marking", fm: "Marking") -> str:
        ...
        try:
            ...
            output = BytesIO()
            pnml_exporter.apply(
                net, im, output, variant=pnml_exporter.Variants.PNML, final_marking=fm
            )
            ...
            logger.error(
                "pnml_export_failed", error=str(e), net_name=getattr(net, "name", "unknown")
            )
            raise

    def export_with_layout(
        self, net: "PetriNet", im: "Marking", fm: "Marking", layout: dict[str, tuple[float, float]]
    ) -> str:
