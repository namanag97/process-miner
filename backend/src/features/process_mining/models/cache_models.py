...
class DFGCache(Base):
    ...
    __table_args__ = (
        Index("ix_dfg_cache_dataset", "dataset_id", unique=True),
    )
...
class VariantCache(Base):
    ...
    avg_variant_length: Mapped[float | None] = mapped_column(
        Integer, nullable=True
    )
    ...
    __table_args__ = (
        Index("ix_variant_cache_dataset", "dataset_id", unique=True),
    )
