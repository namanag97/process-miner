...
class ProcessModel(Base):
    ...
    miner_type: Mapped[str | None] = mapped_column(String(50), nullable=True)  # Deprecated: use model_type
    standard_content_path: Mapped[str | None] = mapped_column(String(500), nullable=True)  # Deprecated: use storage_path
    graph_structure_json: Mapped[str | None] = mapped_column(Text, nullable=True)  # Deprecated: use graph_json
...
class ProcessModelMetrics(Base):
    ...
    __table_args__ = (
        Index("ix_model_metrics_model_dataset", "model_id", "dataset_id"),
    )
...
class GraphCache(Base):
    ...
    __table_args__ = (
        Index("ix_graph_cache_unique", "model_id", "abstraction_level", "layout_algorithm", "filter_params_hash", unique=True),
    )
